	...
	return (
		<Form
			...
			onFinishFailed={(errors) =>
				_.map(errors.errorFields, (errorObj) =>
					notification.warning({ message: errorObj.errors[0] })
				)
			}
			onFinish={onFinish}
			className='custom-form'
		...
		>
			...
			<Form.Item
				rules={[{ required: true, message: 'National Document is required.' }]}
			...
			>
				<FormUploadFile
					...
					onRemove={onRemove}
					imageURL={
						imageURL.national_doc
							? `${app.config.STATIC_SERVER_URL}/media/${imageURL.national_doc}`
							: null
					}
				/>
			</Form.Item>
			<Form.Item
				rules={[{ required: true, message: 'Face Document is required.' }]}
				label='Face Photo'
				name='face_doc'
			>
				<FormUploadFile
					...
					imageURL={
						imageURL.face_doc
							? `${app.config.STATIC_SERVER_URL}/media/${imageURL.face_doc}`
							: null
					}
				/>
			</Form.Item>
			<Form.Item>
				<Row gutter={[16, 0]}>
					<Col>
						<Button size='large' className='custom-button' htmlType='submit'>
							Submit
						</Button>
					</Col>
					<Col>
...
const EventList = ({ events }) => {
	...
	const generateQR = async (ticket) => {
		try {
			...
			// console.log(ticket);
			let url = `${
				app.config.TICKET_VERIFY_URL
			}/admin/event/ticket/verify?user_id=${user.user_id}&ticketId=${
				ticket.ticketId
