				...
				for (let column = 1; column <= totalColumns; column++) {
					if (row <= 5 || row >= totalRows - 4) {
						...
						if (column <= 6 || column >= totalColumns - 4) {
							columnsElement.push(
								<span className='ticket-col ticket-dot'>...</span>
							);
						}
					}
			...
			buttonElements.push(
				<Button
					...
					onClick={(e) => {
						...
						document.getElementsByClassName(`area-${area}`)[0].style.display =
							'block';
					}}
				>
					{area}
	...
	return (
		<Row justify='center'>
			<Col>
				...
				<Input
					onChange={(e) => {
						setRows(_.toInteger(e.target.value));
					}}
				/>
				Col:{' '}
				<Input
					onChange={(e) => {
						setColumns(_.toInteger(e.target.value));
					}}
				/>
			...
			<Row>
				<Col>
					<Button onClick={async () => eventAPI.autoCreateTickets(ticketList)}>
						Submit
					</Button>
				</Col>
			</Row>
